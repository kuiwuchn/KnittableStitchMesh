	...
	build(polygonSoup) {
		...
		for (let I = 0; I < indices.length; I += 1) {
			...
			for (let J = 0; J < faceSize; J++) {
				let h = new Halfedge();
				this.halfedges[halfedgeCount + J] = h;
			}
			...
			for (let J = 0; J < faceSize; J++) {
				...
				let h = this.halfedges[halfedgeCount + J];
				h.next = this.halfedges[halfedgeCount + K];
				h.prev = this.halfedges[halfedgeCount+ (J + faceSize - 1) % faceSize];
				h.onBoundary = false;
		...
		if (this.hasIsolatedVertices()) {
			alert("hasIsolatedVertices!");
			return false;
		}

		if (this.hasIsolatedFaces()) {
			alert("hasIsolatedFaces!");
			return false;
		}

		if (this.hasNonManifoldVertices()) {
			alert("hasNonManifoldVertices!");
			return false;
		}
